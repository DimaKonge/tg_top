...
export function CommunityAvatar({
  ...
  const avatarSrc = candidateIndex < candidates.length ? candidates[candidateIndex] : null;
  ...
  return (
    <span className={`relative ${sizeClasses} grid shrink-0 place-items-center overflow-hidden border border-white/10 bg-[#151f2e] text-sm font-semibold text-slate-200`}>
      {/* Branded Fallback (always present beneath the image) */}
      <span className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_35%_22%,#254e7a_0%,#111720_70%)] p-[20%]">
        <TgTopAnimatedPyramidAvatar className="h-full w-full" title="TG TOP" />
      </span>

      {allowAnimatedMedia && group.animatedAvatarUrl && !videoFailed ? (
        <video
          key={group.animatedAvatarUrl}
...
export function FullBleedCommunityArtwork({ group, allowAnimatedMedia = false }: { group: CommunityArtworkGroup; allowAnimatedMedia?: boolean }) {
  ...
  return (
    <span className="absolute inset-0 overflow-hidden bg-[radial-gradient(circle_at_50%_20%,#253a58_0%,#111720_68%)]">
      {/* Branded Fallback */}
      <span className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_35%_22%,#254e7a_0%,#111720_70%)] p-[24%]">
        <TgTopAnimatedPyramidAvatar className="h-full w-full" title="TG TOP" />
      </span>
      ...
      {allowAnimatedMedia && group.animatedAvatarUrl && !videoFailed ? (
        <video
          ...
          preload="metadata"
          disablePictureInPicture
          className="pointer-events-none absolute inset-0 h-full w-full select-none object-cover transition-transform duration-300 group-hover:scale-105 [-webkit-touch-callout:none]"
          onLoadedData={event => { void event.currentTarget.play().catch(() => undefined); }}
          onError={() => setVideoFailed(true)}
        />
      ) : avatarSrc ? (
